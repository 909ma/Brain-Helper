...
## 프로젝트 개선점
* ANSI만 사용이 가능하다.  
  tsv형식의 데이터베이스를 이용하기에 OS의 text editer를 사용하여 읽는데 Windows 계열과 Linux, Mac OS 계열이 사용하는 형식인 ANSI와 UTF-8의 줄바꿈 처리 문제로 인해 ANSI 형식만 사용 가능합니다.  

* 제한된 서식 스타일을 가진다.  
  문제의 설명에서 '.', '?'이면 일단 한 줄 띄우기 때문에 제한된 서식 양식을 가집니다. 문제 내용에서만 제한을 가지며 정답란에서는 문제가 없습니다.  
  <dir>  
...
* 단순 텍스트 출력이기 때문에 매트릭스, 그래프 등의 정보는 담지 못함.  
